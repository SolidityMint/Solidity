...
contract SolidityMint is Ownable {
    ...
    function deposit(address tokenAddress, uint256 tokenId, uint256 amount, uint256 stake, uint256 gas, uint256 hashrate, uint256 orderId) public payable {
        require(amount > 0, "SolidityMint: amount must be greater than zero");
        require(stake > 0, "SolidityMint: stake must be greater than zero");
        require(msg.value >= gas, "SolidityMint: value must be greater than gas");

        if (tokenAddress == address(0)) {
            require(msg.value >= amount.add(gas), "SolidityMint: value must be greater than amoutn + gas");
        } else {
            require(_tokens[tokenAddress] > 0, "SolidityMint: token is not supported");
            require(tokenAddress.isContract(), "SolidityMint: tokenAddress is not contract");
    ...
    function withdraw(address tokenAddress, uint256 tokenId, uint256 amount, uint256 gas, uint256 orderId) public payable {
        ...
        require(balance >= amount, "SolidityMint: insufficient balance");
        ...
        require(stake > 0, "SolidityMint: insufficient stake");
    ...
    function addToken(address token) public onlyOwner {
        require(token != address(0), "SolidityMint: token the zero address");
        require(token.isContract(), "SolidityMint: token is not contract");
    ...
    function removeToken(address token) public onlyOwner {
        ...
        require(balance == 0, "SolidityMint: token balance must be equal to zero");
    ...
    function setFeeAddress(address _feeAddress) public onlyOwner {
        require(_feeAddress != address(0), "SolidityMint: new feeAddress the zero address");
        feeAddress = _feeAddress;
    }

    function() external payable {
        revert("SolidityMint: does not accept payments");
    }
